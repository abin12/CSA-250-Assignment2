...
if __name__ == "__main__":
    ...
    train_images_d = train_images.reshape((train_images.shape[0], 28*28))
    test_images_d = test_images.reshape((test_images.shape[0], 28*28))
    train_images_c = train_images.reshape((train_images.shape[0], 28, 28, 1))
    test_images_c = test_images.reshape((test_images.shape[0], 28, 28, 1))

    from train_multi_layer import create_MLP
    model_MLP = create_MLP()
    model_MLP.load_weights('./models/dnn_model.h5')
    ...
    loss, gt, pred = test(model_conv_net, test_images_c, test_labels)
